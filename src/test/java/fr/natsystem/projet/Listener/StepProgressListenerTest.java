...
class StepProgressListenerTest {
    ...
    void setUp() {
        ...
        when(stepExecution.getExecutionContext())
                .thenReturn(stepExecutionContext);
    ...
    void shouldReturnInvalidCsvWhenFlatFileParseExceptionIsPresent() {
        FlatFileParseException parseException =
                mock(FlatFileParseException.class);

        when(stepExecution.getFailureExceptions())
                .thenReturn(List.of(parseException));
        ...
        assertEquals(
                "INVALID_CSV",
                jobExecutionContext.getString("csvStatus")
        );
    }
    ...
    void shouldReturnInvalidCsvWhenFlatFileParseExceptionIsNested() {
        ...
        RuntimeException firstWrapper = new RuntimeException(
                "Erreur intermédiaire",
                parseException
        );

        IllegalStateException secondWrapper = new IllegalStateException(
                "Erreur globale",
                firstWrapper
        );

        when(stepExecution.getFailureExceptions())
                .thenReturn(List.of(secondWrapper));
    ...
    void shouldReturnCurrentExitStatusForNonCsvFailure() {
        ...
        RuntimeException failure =
                new RuntimeException("Erreur technique");
    ...
    void shouldFindCsvExceptionAmongSeveralFailures() {
        RuntimeException firstFailure =
                new RuntimeException("Première erreur");

        FlatFileParseException parseException =
                mock(FlatFileParseException.class);

        RuntimeException csvFailure = new RuntimeException(
