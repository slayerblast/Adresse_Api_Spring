...
class CsvToStagingTaskletTest {
    ...
    void setUp() throws Exception {
        tasklet = new CsvToStagingTasklet(
                jdbcTemplate,
                dataSource
        );
        ...
        when(pgConnection.getCopyAPI())
                .thenReturn(copyManager);
    }
    ...
    void shouldImportAdresseCsv() throws Exception {
        ...
        verify(copyManager).copyIn(
                argThat(sql ->
                        sql.contains("COPY adresse_staging")
                                && sql.contains("DELIMITER ';'")
                ),
                any(Reader.class)
        );
    ...
    @Test
    void shouldImportAdresseCsvByDefaultWhenInnerJobIsUnknown()
            throws Exception {
