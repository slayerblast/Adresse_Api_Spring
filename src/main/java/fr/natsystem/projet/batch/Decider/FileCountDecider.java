...
public class FileCountDecider implements JobExecutionDecider {
  ...
  public FlowExecutionStatus decide(
    ...
    if (!inputFile.isBlank()) {
      ...
      result =
          new FlowExecutionStatus(
              "NO_INPUT_FILE"); // le fichier correspondant à l'argument n'est pas trouvé + param de
      // récuperation désactivé
    } else if (count == 1 && !retriever) {
      result =
          new FlowExecutionStatus(
              "OK_FOR_IMPORT"); // un fichier trouvé + param de récuperation désactivé donc on lit
      // le fichier
      ...
      result =
          new FlowExecutionStatus(
              "MULTIPLE_FILES_FOUND"); // un fichier trouvé + param de récuperation activé mais
      // preshot de l'erreur MULTIPLE_FILES_FOUND
    } else if (count == 0 && retriever) {
      result =
          new FlowExecutionStatus(
              "OK_FOR_RETRIEVE"); // Aucun fichier trouvé + param de récuperation activé donc OK
      // pour la suite
    }
