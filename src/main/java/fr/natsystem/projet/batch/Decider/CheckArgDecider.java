...
public class CheckArgDecider implements JobExecutionDecider {
  ...
  public FlowExecutionStatus decide(
    ...
    if (file != null) {
      ...
      count =
          Arrays.stream(Objects.requireNonNull(folder.listFiles())).filter(File::isFile).count();
    }
    ...
    if (fileExist) {
      ...
      // activé donc preshot de l'erreur MULTIPLE_FILES_FOUND
    } else {
      result =
          new FlowExecutionStatus(
              "OK_FOR_RETRIEVE"); // il y a 0 fichier et le paramètre de récupération est activé
    }
