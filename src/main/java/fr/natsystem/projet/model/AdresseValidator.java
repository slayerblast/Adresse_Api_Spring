...
public class AdresseValidator implements Validator<Adresse> {
  ...
  /**
   * à adapter selon le critère choisi !!! Utilisé pour vérifier le format de l'id et la validation
   * de l'INSEE l'id doit commencer par le code l'INSEE ensuite je ne gère pas les caractères
   *
   * @param adresse object to be validated
   * @throws ValidationException
   */
  @Override
  public void validate(Adresse adresse) throws ValidationException {
    ...
    if (!adresse.id().startsWith(codeInsee)) {
      ...
      throw new ValidationException("L'id ne commence pas par le code INSEE : " + codeInsee);
    }
  }
}
